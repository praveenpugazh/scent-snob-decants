import { useState, useEffect } from 'react';
import { S } from '../styles/theme.js';
import { FLAGS } from '../config/flags.js';
import { calc20ml } from '../data/products.js';

export default function SizeModal({ product, onClose, onAdd }) {
  const [sel, setSel] = useState(null);

  useEffect(() => {
    if (!product) return;
    if (product.p5) setSel({ size: '5ml', price: product.p5 });
    else if (product.p10 && FLAGS.ENABLE_10ML) setSel({ size: '10ml', price: product.p10 });
    else if (product.p10 && FLAGS.ENABLE_20ML) setSel({ size: '20ml', price: calc20ml(product.p10) });
  }, [product]);

  if (!product) return null;

  const p20 = calc20ml(product.p10);

  return (
    <div style={S.modalOverlay} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={S.modalPanel}>
        <div style={S.modalBrand}>{product.brand}</div>
        <div style={S.modalName}>{product.name}</div>
        <div style={S.modalNotes}>{product.notes}</div>

        <div style={{ ...S.sizeGrid, gridTemplateColumns: FLAGS.ENABLE_20ML ? '1fr 1fr 1fr' : '1fr 1fr' }}>

          {/* 5ml */}
          {product.p5 ? (
            <div
              style={{ ...S.sizeOpt, ...(sel?.size === '5ml' ? S.sizeOptSel : {}) }}
              onClick={() => setSel({ size: '5ml', price: product.p5 })}
            >
              <span style={S.sizeSz}>5ml</span>
              <span style={S.sizeSp}>₹{product.p5}</span>
            </div>
          ) : (
            <div style={{ ...S.sizeOpt, opacity: 0.4, cursor: 'not-allowed' }}>
              <span style={S.sizeSz}>5ml</span>
              <span style={{ ...S.sizeSp, fontSize: 11, color: '#b09060', fontStyle: 'italic' }}>N/A</span>
            </div>
          )}

          {/* 10ml — feature flagged */}
          {FLAGS.ENABLE_10ML ? (
            product.p10 ? (
              <div
                style={{ ...S.sizeOpt, ...(sel?.size === '10ml' ? S.sizeOptSel : {}) }}
                onClick={() => setSel({ size: '10ml', price: product.p10 })}
              >
                <span style={S.sizeSz}>10ml</span>
                <span style={S.sizeSp}>₹{product.p10}</span>
              </div>
            ) : (
              <div style={{ ...S.sizeOpt, opacity: 0.35, cursor: 'not-allowed' }}>
                <span style={S.sizeSz}>10ml</span>
                <span style={{ ...S.sizeSp, fontSize: 11, color: '#b09060', fontStyle: 'italic' }}>N/A</span>
              </div>
            )
          ) : (
            <div style={{ ...S.sizeOpt, opacity: 0.35, cursor: 'not-allowed', background: 'rgba(176,144,96,0.03)' }}>
              <span style={S.sizeSz}>10ml</span>
              <span style={{ ...S.sizeSp, fontSize: 11, color: '#b09060', fontStyle: 'italic' }}>Glass · Soon</span>
            </div>
          )}

          {/* 20ml — separate feature flag */}
          {FLAGS.ENABLE_20ML ? (
            p20 > 0 ? (
              <div
                style={{ ...S.sizeOpt, ...(sel?.size === '20ml' ? S.sizeOptSel : {}) }}
                onClick={() => setSel({ size: '20ml', price: p20 })}
              >
                <span style={S.sizeSz}>20ml</span>
                <span style={S.sizeSp}>₹{p20}</span>
                <span style={{ fontSize: 10, color: '#b09060', fontStyle: 'italic', display: 'block', marginTop: 2 }}>Best value</span>
              </div>
            ) : (
              <div style={{ ...S.sizeOpt, opacity: 0.35, cursor: 'not-allowed' }}>
                <span style={S.sizeSz}>20ml</span>
                <span style={{ ...S.sizeSp, fontSize: 11, color: '#b09060', fontStyle: 'italic' }}>N/A</span>
              </div>
            )
          ) : (
            <div style={{ ...S.sizeOpt, opacity: 0.35, cursor: 'not-allowed', background: 'rgba(176,144,96,0.03)' }}>
              <span style={S.sizeSz}>20ml</span>
              <span style={{ ...S.sizeSp, fontSize: 11, color: '#b09060', fontStyle: 'italic' }}>Coming soon</span>
            </div>
          )}

        </div>

        <div style={S.modalBtns}>
          <button style={S.btnCancel} onClick={onClose}>Cancel</button>
          <button
            style={{ ...S.btnAdd, opacity: sel ? 1 : 0.5 }}
            onClick={() => sel && onAdd(product, sel)}
            disabled={!sel}
          >
            Add to Cart
          </button>
        </div>
      </div>
    </div>
  );
}
