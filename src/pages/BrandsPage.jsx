...
import { allProducts, BRANDS_NICHE, BRANDS_DESIGNER, BRANDS_DUPES } from '../data/products.js';
...
function BrandCard({ brand, count, cat, onClick }) {
  ...
  return (
    <div
      style={{
        background:   hov ? 'rgba(176,144,96,0.08)' : C.bg3,
      ...
      onClick={onClick}
    >
      <div style={{ fontSize: 9, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#b09060' }}>
        {catLabel}
      </div>
      <div style={{ fontFamily: 'var(--ff-serif)', fontSize: '1.2rem', fontWeight: 300, color: C.t1, lineHeight: 1.2 }}>
        {brand}
      </div>
      ...
      <div style={{
        ...
        <button
          style={{
            ...
            fontFamily: 'var(--ff-sans)', display: 'flex', alignItems: 'center',
        ...
        <div style={{ fontSize: 9, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#b09060', marginBottom: 8 }}>
          {catLabel}
        </div>
        <h1 style={{ fontFamily: 'var(--ff-serif)', fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 300, color: C.t1, marginBottom: 8 }}>
          {brand}
        </h1>
...
export default function BrandsPage({ onOpen }) {
  ...
  // Show brand-specific page if selected
  if (selectedBrand) {
    return (
      <BrandProductsPage
        brand={selectedBrand}
  ...
  return (
    <div style={{ minHeight: '100vh', paddingTop: '6rem' }}>
      ...
      <div style={{ padding: '1.5rem 2rem 0' }}>
        <div style={{
          ...S.searchWrap,
          ...(focused ? { borderColor: 'rgba(176,144,96,0.5)', boxShadow: '0 0 0 3px rgba(176,144,96,0.07)' } : {}),
        }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={C.t3} strokeWidth="1.5">
          ...
          />
          {query && (
            <button style={{ background: 'none', border: 'none', color: C.t3, cursor: 'pointer', fontSize: 16, lineHeight: 1 }}
              onClick={() => setQuery('')}>
              ✕
            </button>
          )}
      ...
      {isSearching && searchedProducts.length > 0 && (
        <div style={{ padding: '0 2rem 1.5rem' }}>
          ...
          <div style={S.grid}>
            {searchedProducts.map(p => <ProductCard key={p.id} product={p} onOpen={onOpen} />)}
          </div>
        </div>
      )}
      ...
      {(!isSearching || filteredBrands.length > 0) && (
        <div style={{ padding: '1rem 2rem 2rem' }}>
          ...
          {isSearching && filteredBrands.length > 0 && (
            <div style={{ ...S.countLbl, marginTop: '0.5rem' }}>
              {filteredBrands.length} brand{filteredBrands.length !== 1 ? 's' : ''} matching "{query}"
            </div>
          )}
          {!isSearching && (
            <div style={S.countLbl}>{filteredBrands.length} brand{filteredBrands.length !== 1 ? 's' : ''}</div>
          )}
