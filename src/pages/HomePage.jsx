  ...
  return (
    <div>
      ...
      <section style={S.hero}>
        ...
        <h1 style={S.heroH1}>Scent<br/>Snob <em style={S.heroEm}>Decants</em></h1>
        <p style={S.heroSub}>Niche · Designer · Middle Eastern Dupes · 5ml</p>
        <div style={S.heroBtns}>
          <button style={S.btnGold}    onClick={() => setPage('brands')}>Explore Collection</button>
          <button style={S.btnOutline} onClick={() => setPage('about')}>Our Story</button>
        </div>
      ...
      <section style={{ ...S.section, background: C.bg1 }}>
        <div style={S.sHdr}>
          <h2 style={S.sH2}>Staff <em style={S.sH2em}>Picks</em></h2>
          <button style={S.sLink} onClick={() => setPage('brands')}>Browse all →</button>
        </div>
          ...
          {FEATURED.map(p => (
            <div key={p.id} style={S.featCard} onClick={() => onOpen(p)}>
              <div style={S.featImg}>
                ...
                <div style={{ ...S.featBadge, background: p.cat==='niche' ? 'rgba(255,255,255,0.10)' : 'rgba(176,144,96,0.30)', color:'rgba(255,255,255,0.8)' }}>
              ...
              <div style={S.featBody}>
                ...
                <div style={S.featFooter}>
                  <div>
                    {p.p5 && <div style={S.pr}>₹{p.p5} <span style={{ color:C.t3, fontSize:10 }}>/ 5ml</span></div>}
        ...
        <div style={{ display:'flex', justifyContent:'center', gap:8, marginTop:16 }}>
          {[[-1,'←'],[1,'→']].map(([d,l]) => (
            <button key={d} onClick={() => scrollCar(d)} style={{ width:36, height:36, border:`0.5px solid ${C.border}`, background:C.bg3, borderRadius:2, cursor:'pointer', color:C.t2, fontSize:14 }}>{l}</button>
          ))}
        </div>
      </section>

      {/* PROMISES */}
      <section style={{ ...S.section, background: C.bg2, paddingTop:'2rem', paddingBottom:'2rem' }}>
        <div style={{ ...S.promiseRow, gridTemplateColumns:'repeat(auto-fit, minmax(140px, 1fr))' }}>
          {[['◈','Authentic','Genuine decants, no dilution'],['◎','Sealed','Freshly decanted & sealed'],['◇','Discreet','Secure padded shipping'],['✦','Fast Ship','3–7 days PAN India']].map(([icon,title,sub]) => (
